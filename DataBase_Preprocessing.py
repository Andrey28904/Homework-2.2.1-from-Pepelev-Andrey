...
class DB_Preprocessing:
    ...
    def try_to_get_vacancy(self, line: list) -> Vacancy_Small or None:
        ...
        if is_valid_cur and (sal_from or sal_to):
            try:
                ...
                df = pd.read_sql(f"SELECT {line_cur} FROM currencies WHERE Data = '{data}'", connect)
                currency_value = float(df.values[0][0])
    ...
    def create_db_from_data(self, data: list) -> None:
        """Создание базы данных из полученных данных.
        Args:
            data(list): список всех вакансий в виде списка значений каждой вакансии.
        """
        temp_file_name = self.vac_db_path+"/"+"temp.csv"
        with open(file=temp_file_name, mode="w", encoding="utf-8-sig", newline='') as csv_basic_file:
            csv_base = csv.writer(csv_basic_file)
            csv_base.writerows([DB_Preprocessing.new_needed_fields] + data)
    ...
    def read_csv_and_create_db(self) -> None:
        ...
        with open(self.vacancy_csv_path, "r", encoding='utf-8-sig', newline='') as csv_file:
            ...
            for line in file:
                ...
                if vac is not None:
                    all_new_lines.append(vac.get_list())
                    print(vac.get_list())
        csv_file.close()
        self.create_db_from_data(all_new_lines)
